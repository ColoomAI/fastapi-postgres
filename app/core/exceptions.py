...
state (third-party API down, "project paused", etc). The handler will:
...
Same semantics as express-postgres's `httpErr({expected})` — keep the
two stacks aligned so the AI scaffolding new endpoints sees one shape
across languages.
"""
...
class HTTPError(Exception):
    ...
    Args:
        ...
        code: Optional structured code (e.g. "ERR_AUTH_REQUIRED") for
            the frontend to switch on without parsing the message.
